/**
 * DEPRECATED use edo.modal.loadAjax instead
 * Function to ajax load modals. Used by gridRowActions
 * @deprecated
 */
function ajaxLoadModal(loadUrl, modalContainerId, modalId) {
    var hashModalId = "#" + modalId;
    var hashModalContainerId = "#" + modalContainerId;
    $(hashModalContainerId).load(loadUrl, function() {
        $(hashModalId).modal("show");
        $(hashModalId).on("shown.bs.modal", function() {
            $.EventBus("initializeWidgets").publish(modalId);
        });
        $(hashModalId).on("hidden.bs.modal", function () {
            $(hashModalContainerId).empty();
        });
    });
}
...
$(document).on("click.bs.modal.data-api", "[data-toggle='modal-ajax']", function (e) {
    ...
    var callbacks = {};
    $.each(edo.modal.EVENTS, function(i, callback) {
        callbacks[callback] = edo.util.dereference($this.data(callback), "function");
    });
    ...
    edo.modal.loadAjax(url, params, callbacks);
});
